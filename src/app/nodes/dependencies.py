...
def get_node_publisher(
    rabbit: Annotated[RabbitMQ, Depends(get_rabbitmq)],
) -> NodePublisher:
    return NodePublisher(rabbit.channel)


def get_node_service(
    session: Annotated[AsyncSession, Depends(get_async_session)],
    health_check_service: Annotated[NodeHealthCheckService, Depends(get_health_check_service)],
    publisher: Annotated[NodePublisher, Depends(get_node_publisher)],
) -> NodeService:
    repository = NodeRepository(session)
    return NodeService(repository, health_check_service, publisher)
